#!/usr/bin/env python3
"""Small test script for knapsack experiment - 100 agents, 50 items."""
import os
import time
import numpy as np
import pandas as pd
...
def main():
    ...
    if rank == 0:
        print(f"Testing knapsack with {num_agents} agents, {num_items} items, {num_features} features")
        print("=" * 80)
    ...
    prepared = mpi_call_with_timeout(
        comm, prepare_scenario, timeout_seconds, "knapsack-prep-test"
    )
    ...
    obs_bundles = mpi_call_with_timeout(
        comm, generate_bundles, timeout_seconds, "knapsack-bundles-test"
    )
    ...
    theta_row = mpi_call_with_timeout(
        comm, solve_row_gen, timeout_seconds, "knapsack-rg-test"
    )
    ...
    theta_row1 = mpi_call_with_timeout(
        comm, solve_row_gen_1slack, timeout_seconds, "knapsack-rg1-test"
    )
