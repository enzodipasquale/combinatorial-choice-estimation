#!/usr/bin/env python3
"""Small test script for supermod experiment - 100 agents, 50 items."""
import os
import time
import numpy as np
import pandas as pd
...
def main():
    ...
    if rank == 0:
        print(f"Testing supermod with {num_agents} agents, {num_items} items, {num_features} features")
        print("=" * 80)
    ...
    theta_row = mpi_call_with_timeout(
        comm, solve_row_gen, timeout_seconds, "supermod-rg-test"
    )
    ...
    theta_row1 = mpi_call_with_timeout(
        comm, solve_row_gen_1slack, timeout_seconds, "supermod-rg1-test"
    )
