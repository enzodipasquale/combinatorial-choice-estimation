...
from combest.subproblems.solver_base import SubproblemSolver, create_gurobi_model, apply_solver_settings
...
class TwoStageSolverNN(SubproblemSolver):
    ...
    def initialize(self):
        ...
        input_lb = np.concatenate([np.zeros(M), ckpt["theta_lb"]])
        input_ub = np.concatenate([np.ones(M), ckpt["theta_ub"]])
        self.pre_bounds = compute_layer_bounds(
            self.nn_weights, self.nn_biases, input_lb, input_ub)
        n_theta = len(ckpt["theta_lb"])

        # Surrogate MIP per agent (b_1 + embedded NN)
        ...
        for i in range(n):
            ...
            nn_out = embed_relu(m, self.nn_weights, self.nn_biases,
                                list(b_1.tolist()) + tvars, self.pre_bounds)
            self.surrogate_models.append(m)
            self.b_1_vars.append(b_1)
            self.theta_vars.append(tvars)
            self.nn_out_vars.append(nn_out)

        # EntryProblem per agent (reuse for second-stage knapsacks)
        self.problems = []
        for i in range(n):
            self.problems.append(
                EntryProblem(M, self.R, self.cap[i], self.subproblem_cfg))
    ...
    def update_solver_settings(self, settings_dict):
        all_models = self.surrogate_models + [ep.q_model for ep in self.problems]
        apply_solver_settings(all_models, settings_dict)
