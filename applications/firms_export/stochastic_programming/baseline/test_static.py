#!/bin/env python
"""
Static (beta=0) estimation using min-cut solver.
Verifies gradients match the TwoStageSolver at beta=0.
"""
import sys
...
# ── Build input data with both SP and min-cut keys ────────────────────
def build_combined_input(ctx, R):
    rev_1 = ctx["rev_chars_1"][:, 0, :]            # (n, M)
    state = ctx["state_chars"]                       # (n, M)
    entry_chars = ctx["entry_chars"]                 # (M,) d_j thousands km
    C = ctx["syn_chars"]                             # (M, M) exp(-dist_jk), zero diag
    switch = 1 - state                               # (n, M)

    # Synergy: new entry j benefits from existing dest k
    syn = state @ C                                  # (n, M)
    ...
    return {
        "id_data": {
            ...
            # min-cut solver keys (purely modular now)
            "modular":          modular,
...
# Workaround: min_cut.py uses self.data_manager.id_data instead of
# self.data_manager.local_data.id_data (bug in combest)
model.data_manager.id_data = model.data_manager.local_data.id_data


def setup_mincut_errors(model, seed):
    """Set up errors matching baseline seeding for min-cut solver."""
...
for theta in thetas:
    ...
    for seed in error_seeds:
        # ── Min-cut (purely modular, no quadratic) ──────────────
