...
def plot(raw, ctx):
    ...
    ax.hist(elig[win],  bins=bins, color=NAVY,  alpha=0.85, label="Winners",
    ...
    fig.tight_layout()
    fig.savefig(OUT_FIG / "fig_heterogeneity_hist.png", dpi=DPI, bbox_inches="tight")
    plt.close(fig)
    print(f"  {len(elig)} bidders, {win.sum()} winners; "
          f"elig {elig.min():,.0f}–{elig.max():,.0f}")
