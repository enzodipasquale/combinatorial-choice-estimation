...
# If set, compute mean subgradients using local feature sums + MPI Allreduce (avoids gather of N x K)
USE_ALLREDUCE_MEAN = os.environ.get("USE_ALLREDUCE_MEAN", "1") == "1"
...
def compute_avg_subgradient_beta(theta, errors_all_sims):
    ...
    Fast path (default): avoids gathering per-agent features to rank 0 by
    computing local sums of feature vectors and using MPI Allreduce.

    This is valid because g_bar only needs the mean over agents, not the
    per-agent array.
    """
    if not USE_ALLREDUCE_MEAN:
        g_i = compute_per_agent_subgradients(theta, errors_all_sims)
        if rank == 0:
            return g_i[:, beta_indices].mean(axis=0)  # (num_beta,)
        return None
...
if rank == 0:
    print(f"\nComputing B matrix ({NUM_SIMULS_SE} simulations)...")
