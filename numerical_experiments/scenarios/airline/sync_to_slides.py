...
def aggregate_mc(mc):
    ...
    # RMSE = sqrt(bias^2 + std^2) (exact identity for this sample variance).
    def rmse(bias_val, std_val):
        return float(np.sqrt(bias_val ** 2 + std_val ** 2))

    return {
        "rev":  {"true": float(true[i_rev]),  "bias": float(bias[i_rev]),
                 "std":  float(std[i_rev]),
                 "rmse": rmse(float(bias[i_rev]), float(std[i_rev]))},
        "cong": {"true": float(true[i_gs]),   "bias": float(bias[i_gs]),
                 "std":  float(std[i_gs]),
                 "rmse": rmse(float(bias[i_gs]), float(std[i_gs]))},
        "fc":   {"true": float(true_fc_avg),  "bias": float(bias_fc_avg),
                 "std":  float(std_fc_avg),
                 "rmse": rmse(float(bias_fc_avg), float(std_fc_avg))},
    }
...
def row_labels_for(n_firms):
    return ROW_LABELS_SINGLE if n_firms == 1 else ROW_LABELS_MULTI


def build_mc_table(agg, n_firms):
    lines = [
        ...
        "% True = fixed theta*, Bias and Std across replications.",
        r"\begin{tabular}{@{}l rrr@{}}",
        r"\toprule",
        r" & True & Bias & Std \\",
        r"\midrule",
    ]
    for key, label in row_labels_for(n_firms):
        a = agg[key]
        lines.append(
            f"{label} & {fmt(a['true'])} & {fmt_signed(a['bias'])} "
            f"& {fmt(a['std'])} " + r"\\")
    lines += [r"\bottomrule", r"\end{tabular}"]
    return "\n".join(lines) + "\n"


def build_bootstrap_table(agg, n_firms):
    lines = [
        "% Auto-generated by numerical_experiments/scenarios/airline/sync_to_slides.py",
...
def build_runtime_macros(mc, boot):
    ...
    lines = [
        ...
        rf"\newcommand{{\airlineN}}{{{mc['config']['N']}}}",
        rf"\newcommand{{\airlineC}}{{{mc['config']['C']}}}",
        rf"\newcommand{{\airlineM}}{{{mc['config']['M']}}}",
        rf"\newcommand{{\airlineNParams}}{{{mc['config']['n_params']}}}",
    ]
...
def main():
    ...
    # Tables
    (TABLES / "tab_airline_mc_full.tex").write_text(
        build_mc_table(mc_agg, n_firms))
    (TABLES / "tab_airline_mc_bootstrap.tex").write_text(
        build_bootstrap_table(boot_agg, n_firms))
