...
def solve_QS(self, lambda_k, p_j, num_iters_SGD, alpha, method = 'constant_step_lenght'):
    ...
    error_i_j = self.local_errors
    modular_i_j_k = self.local_agent_data["modular"]
    quadratic_i_j_j_k = self.local_agent_data["quadratic"]
    quadratic_j_j_k = self.item_data["quadratic"]

    num_MOD = modular_i_j_k.shape[-1]

    z_t = np.full((self.num_local_agents, self.num_items), 0.5)
    z_best_i_j = np.zeros_like(z_t)
    val_best_i = np.full_like(z_t, -np.inf)
