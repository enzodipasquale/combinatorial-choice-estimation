...
def greedy_fe_oracle(agent_idx: int, bundles: np.ndarray, data: dict):
...
def run_greedy_lowdim(comm):
    ...
    if rank == 0:
        ...
        print('=' * 70)
    NUM_FEATURES = 3
    scenario = ScenarioLibrary.greedy().with_dimensions(num_obs=200, num_items=30).with_num_features(NUM_FEATURES).build()
    prepared = scenario.prepare(comm=comm, timeout_seconds=60, seed=42)
    bc = BundleChoice()
    config = {'dimensions': {'num_obs': 200, 'num_items': 30, 'num_features': NUM_FEATURES, 'num_simulations': 1}, 'subproblem': {'name': 'Greedy'}, 'row_generation': {'max_iters': 100, 'theta_ubs': 100}}
...
def run_greedy_fe(comm):
    ...
    scenario = ScenarioLibrary.greedy().with_dimensions(num_obs=200, num_items=NUM_ITEMS).with_num_features(NUM_AGENT_FEATURES + 1).build()
    prepared = scenario.prepare(comm=comm, timeout_seconds=60, seed=42)
    if rank == 0:
        ...
        est_data = None
    bc = BundleChoice()
    config = {'dimensions': {'num_obs': 200, 'num_items': NUM_ITEMS, 'num_features': NUM_FEATURES, 'num_simulations': 1}, 'subproblem': {'name': 'Greedy'}, 'row_generation': {'max_iters': 100, 'theta_ubs': 100}}
    bc.load_config(config)
...
def run_knapsack_lowdim(comm):
    ...
    if rank == 0:
        ...
        print('=' * 70)
    scenario = ScenarioLibrary.linear_knapsack().with_dimensions(num_obs=150, num_items=20).with_feature_counts(num_agent_features=2, num_item_features=2).build()
    prepared = scenario.prepare(comm=comm, timeout_seconds=120, seed=42)
    bc = BundleChoice()
    config = {'dimensions': {'num_obs': 150, 'num_items': 20, 'num_features': 4, 'num_simulations': 1}, 'subproblem': {'name': 'LinearKnapsack', 'settings': {'TimeLimit': 1.0}}, 'row_generation': {'max_iters': 100, 'theta_ubs': 100}}
    bc.load_config(config)
    bc.data.load_input_data(prepared.estimation_data if rank == 0 else None)
...
def run_knapsack_fe(comm):
    ...
    NUM_ITEMS = 20
    NUM_AGENT_FEATURES = 2
    NUM_FEATURES = NUM_AGENT_FEATURES + NUM_ITEMS + 2
    scenario = ScenarioLibrary.linear_knapsack().with_dimensions(num_obs=150, num_items=NUM_ITEMS).with_feature_counts(num_agent_features=NUM_AGENT_FEATURES, num_item_features=2).build()
    prepared = scenario.prepare(comm=comm, timeout_seconds=120, seed=42)
    if rank == 0:
        est_data = prepared.estimation_data.copy()
        ...
        est_data = None
    bc = BundleChoice()
    config = {'dimensions': {'num_obs': 150, 'num_items': NUM_ITEMS, 'num_features': NUM_FEATURES, 'num_simulations': 1}, 'subproblem': {'name': 'LinearKnapsack', 'settings': {'TimeLimit': 1.0}}, 'row_generation': {'max_iters': 100, 'theta_ubs': 100}}
    bc.load_config(config)
    bc.data.load_input_data(est_data)
    bc.oracles.build_quadratic_features_from_data()
    bc.subproblems.load()
