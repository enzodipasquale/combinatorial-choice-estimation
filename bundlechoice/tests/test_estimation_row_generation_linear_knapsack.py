...
def test_row_generation_linear_knapsack():
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    scenario = ScenarioLibrary.linear_knapsack().with_dimensions(num_obs=20, num_items=20).with_feature_counts(num_agent_features=2, num_item_features=2).with_num_simulations(1).build()
    prepared = scenario.prepare(comm=comm, timeout_seconds=300, seed=33)
