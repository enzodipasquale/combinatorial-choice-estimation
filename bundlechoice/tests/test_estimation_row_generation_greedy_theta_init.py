...
def test_row_generation_greedy_theta_init_same_problem():
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    scenario = ScenarioLibrary.greedy().with_dimensions(num_obs=100, num_items=100).with_num_features(6).with_num_simulations(1).build()
    ...
    if rank == 0:
        ...
        assert num_iterations < num_iterations1 or num_iterations <= 3, f'Expected fewer iterations with theta_init ({num_iterations}) than without ({num_iterations1}), or <= 3'
...
def test_row_generation_greedy_theta_init_helps_convergence():
    ...
    if rank == 0:
        ...
        assert num_iterations3_with_init <= num_iterations2_no_init + 1, f'Initialization should help: {num_iterations3_with_init} > {num_iterations2_no_init} + 1'
