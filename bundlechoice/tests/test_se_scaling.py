"""
Test SE scaling: SE ~ 1/√N
Verifies standard errors decrease as sample size increases.
"""
...
def run_with_sample_size(comm, num_agents, seed):
    """Run estimation and SE for given sample size."""
    rank = comm.Get_rank()
    NUM_FEATURES = 3
    
    scenario = (
        ScenarioLibrary.greedy()
        .with_dimensions(num_agents=num_agents, num_items=30)
    ...
    # SE with more simulations for stability
    ...
    if rank == 0 and se_result is not None:
        return {
            ...
            "se": se_result.se,
            "A_cond": np.linalg.cond(se_result.A_matrix),
