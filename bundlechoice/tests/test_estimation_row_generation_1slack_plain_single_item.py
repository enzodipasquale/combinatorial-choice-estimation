...
def test_row_generation_1slack_plain_single_item():
    ...
    tic = time.time()
    solver = RowGeneration1SlackManager(comm_manager=demo.comm_manager, dimensions_cfg=demo.config.dimensions, row_generation_cfg=demo.config.row_generation, data_manager=demo.data_manager, oracles_manager=demo.oracles_manager, subproblem_manager=demo.subproblem_manager)
    result = solver.solve()
    toc = time.time()
    if rank == 0:
