...
def test_row_generation_vs_1slack_identical_objval():
    ...
    num_simulations = 1
    if rank == 0:
        modular = np.random.normal(0, 1, (num_obs, num_items, num_features - 1))
        errors = np.random.normal(0, 1, size=(num_simulations, num_obs, num_items))
        input_data = {'agent_data': {'modular': modular}, 'errors': errors}
    else:
        input_data = None
    cfg_base = {'dimensions': {'num_obs': num_obs, 'num_items': num_items, 'num_features': num_features, 'num_simulations': num_simulations}, 'subproblem': {'name': 'Greedy'}}
    bc_gen = BundleChoice()
    ...
    cfg_rg['row_generation'] = {'max_iters': 100, 'tolerance_optimality': 1e-06, 'gurobi_settings': {'OutputFlag': 0}}
    bc_rg = BundleChoice()
    ...
    cfg_1s['row_generation'] = {'max_iters': 1000, 'tolerance_optimality': 1e-06, 'gurobi_settings': {'OutputFlag': 0}}
    bc_1s = BundleChoice()
    bc_1s.load_config(cfg_1s)
    bc_1s.data.load_input_data(input_data)
    bc_1s.oracles.set_features_oracle(features_oracle)
    bc_1s.subproblems.load()
    solver_1s = RowGeneration1SlackManager(comm_manager=bc_1s.comm_manager, dimensions_cfg=bc_1s.config.dimensions, row_generation_cfg=bc_1s.config.row_generation, data_manager=bc_1s.data_manager, oracles_manager=bc_1s.oracles_manager, subproblem_manager=bc_1s.subproblem_manager)
    ...
    if rank == 0:
        ...
        assert objval_diff < 1e-06, f'Row Gen and 1-Slack give different objvals: {objval_diff}'
