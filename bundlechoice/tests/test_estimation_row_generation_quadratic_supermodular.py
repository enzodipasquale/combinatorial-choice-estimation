...
def test_row_generation_quadsupermodular():
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    scenario = ScenarioLibrary.quadratic_supermodular().with_dimensions(num_obs=20, num_items=50).with_feature_counts(num_mod_agent=2, num_mod_item=2, num_quad_agent=0, num_quad_item=2).with_num_simulations(1).build()
