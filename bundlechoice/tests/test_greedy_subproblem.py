...
def generate_test_data(num_obs, num_items, agent_modular_dim, item_modular_dim, num_simulations):
    ...
    dimensions_cfg = {'num_obs': num_obs, 'num_items': num_items, 'num_features': num_features, 'num_simulations': num_simulations}
...
def test_greedy_vs_bruteforce():
    num_obs, num_items, num_simulations = (20, 12, 2)
    agent_modular_dim, item_modular_dim = (2, 3)
    input_data, dimensions_cfg = generate_test_data(num_obs, num_items, agent_modular_dim, item_modular_dim, num_simulations)
    cfg = {'dimensions': dimensions_cfg, 'subproblem': {'name': 'Greedy', 'settings': {}}}
    bc = BundleChoice()
    bc.load_config(cfg)
    bc.data.load_input_data(input_data)
    bc.oracles.set_features_oracle(features_oracle)
    test_lambdas = [np.ones(dimensions_cfg['num_features']), np.random.normal(0, 1, dimensions_cfg['num_features']), np.array([1] * (dimensions_cfg['num_features'] - 1) + [0.1])]
