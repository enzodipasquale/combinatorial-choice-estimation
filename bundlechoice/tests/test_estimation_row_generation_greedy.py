...
def test_row_generation_greedy():
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    scenario = ScenarioLibrary.greedy().with_dimensions(num_obs=250, num_items=50).with_num_features(6).with_num_simulations(1).build()
    ...
    if rank == 0:
        ...
        print('aggregate demands:', obs_bundles.sum(1).min(), obs_bundles.sum(1).max())
