"""
Test script for MPI communication optimizations.

Tests broadcast_array_with_flag and concatenate_array_at_root_fast methods
to ensure correctness and catch any bugs introduced by the optimization.
"""
import numpy as np
import pytest
...
def test_broadcast_array_with_flag():
    """Test broadcast_array_with_flag with various scenarios."""
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()
    comm_manager = CommManager(comm)
    
    # Test 1: Basic float64 array with flag=True
    if rank == 0:
    ...
    # Test 2: Large array with flag=False
    array_size = 1000
    if rank == 0:
    ...
    # Test 3: Test error handling - None array on root (skip in MPI test, test separately)
    # Note: Error handling tests that cause MPI abort should be tested separately
    # We'll verify correctness instead
...
def test_concatenate_array_at_root_fast():
    """Test concatenate_array_at_root_fast with various scenarios."""
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()
    comm_manager = CommManager(comm)
    
    # Test 1: 1D float64 arrays
    local_size = 10 + rank  # Different sizes per rank
    local_array = np.arange(local_size, dtype=np.float64) + rank * 100
    
    result = comm_manager.concatenate_array_at_root_fast(local_array, root=0)
    ...
    # Test 2: 2D float64 arrays (different first dimension, same second)
    ...
    # Test 3: Bool arrays (should be packed as uint8 and unpacked)
    local_size = 8 + rank
    ...
    if rank == 0:
        ...
        # Verify correctness with deterministic pattern
...
def test_edge_cases():
    ...
    # Test 3: Empty array concatenation
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_concatenate_array_at_root_fast()
        test_edge_cases()
