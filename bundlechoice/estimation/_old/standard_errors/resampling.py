    ...
    def compute_bootstrap(self, theta_hat, solve_fn, num_bootstrap=100, beta_indices=None, seed=None):
        ...
        for b in range(num_bootstrap):
            ...
            if self.comm_manager._is_root():
                ...
                boot_data = {'obs_bundle': obs_bundles[idx], 'agent_data': {k: v[idx] for k, v in agent_data.items()}, 'errors': np.random.randn(N, self.dimensions_cfg.num_items)}
    ...
    def compute_subsampling(self, theta_hat, solve_fn, subsample_size=None, num_subsamples=100, beta_indices=None, seed=None):
        ...
        for s in range(num_subsamples):
            ...
            if self.comm_manager._is_root():
                ...
                sub_data = {'obs_bundle': obs_bundles[idx], 'agent_data': {k: v[idx] for k, v in agent_data.items()}, 'errors': np.random.randn(b, self.dimensions_cfg.num_items)}
                if item_data is not None:
