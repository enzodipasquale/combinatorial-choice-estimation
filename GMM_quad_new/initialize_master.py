...
def init_master(epsilon_si_j, modular_characteristics_i_j_k, quadratic_characteristic_j_j_k, num_simulations):
    ...
    phi_hat_i_k = np.concatenate((
                        np.einsum('ijk,ij->ik', modular_characteristics_i_j_k, matching_i_j),
    ...
    print_init_master(master_pb, num_characteristics, num_simulations, num_agents, num_objects, phi_hat_k, UB)
    my_print([['Pricing and Master init done. Time: ', datetime.datetime.now().time().strftime("%H:%M:%S")]])

    return master_pb, (lambda_k, u_si, p_j) , slack_counter
