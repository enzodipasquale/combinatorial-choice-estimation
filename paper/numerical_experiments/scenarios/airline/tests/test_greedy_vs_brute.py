"""Step 4/5 verification: greedy must match brute-force at small M.

Mandatory test — runs 10 random airlines x 10 random thetas x random errors.
"""
...
def _run_greedy_vs_brute(C, seed):
    """Run the comparison for a given C (number of cities)."""
    ...
    for t in range(N_THETAS):
        theta_mod = rng_test.uniform(-2, 2, phi.shape[1])
        theta_gs = rng_test.uniform(0.1, 3.0)
        hubs_list = build_hubs(N_AIRLINES, C, np.random.default_rng(seed + t))
        ...
        for i in range(N_AIRLINES):
            ...
            assert v_greedy >= v_brute - TOL, (
                ...
                f"greedy={v_greedy:.8f}, brute={v_brute:.8f}, diff={v_greedy-v_brute:.2e}"
            )


@pytest.mark.parametrize("C", [2, 3])
def test_greedy_matches_brute_force(C):
    """Greedy utility >= brute-force utility - tol for C=2 (M=2) and C=3 (M=6)."""
    _run_greedy_vs_brute(C, seed=42)


def test_greedy_matches_brute_force_origin_fe():
    """Same test with fe_mode='origin'."""
    C = 3
    ...
    for t in range(N_THETAS):
        n_mod = phi.shape[1]
        theta_mod = rng_test.uniform(-2, 2, n_mod)
        theta_gs = rng_test.uniform(0.1, 3.0)
        hubs_list = build_hubs(N_AIRLINES, C, np.random.default_rng(99 + t))
        errors = rng_test.normal(0, 1.0, (N_AIRLINES, M))
        ...
        for i in range(N_AIRLINES):
            ...
            b_brute, v_brute = brute_force_demand(
                phi, theta_mod, theta_gs, hubs_list[i], origin_of, errors[i], M)
            assert v_greedy >= v_brute - TOL, (
                f"Greedy < brute-force (origin FE)! theta={t}, airline={i}: "
