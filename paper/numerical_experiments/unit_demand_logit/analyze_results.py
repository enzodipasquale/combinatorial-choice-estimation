...
def load_cell(J, N):
    path = RESULTS_DIR / f"logit_J{J}_N{N}.npz"
    if not path.exists():
        return None
...
def compute_cell_stats(data):
    ...
    rmse_mle = np.sqrt(np.mean((bm - beta)**2, axis=0))
    rmse_cb = np.sqrt(np.mean((bc - beta)**2, axis=0))
...
def generate_latex_table(all_stats, J_values, N_values, beta, output_path):
    ...
    lines = [
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{Monte Carlo results: Combest vs.\ Logit MLE "
        rf"($K={K}$, $\beta^*=({','.join(f'{b:.1f}' for b in beta)})$, "
        rf"$\sigma={1.0}$, $\rho={0.5}$, $R=200$)}}",
        r"\label{tab:unit_demand_logit_efficiency}",
        r"\small",
...
def generate_figure(all_stats, J_values, N_values, output_path):
    ...
    for ax, J in zip(axes, J_values):
        ...
        ax.plot(Ns, sqrt_n_sd_mle, "o-", label="Logit MLE",
