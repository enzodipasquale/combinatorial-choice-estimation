...
EXPERIMENTS_DIR = BASE_DIR / "01_experiments"
...
def main():
    ...
    for size_name, num_agents, num_items in sizes_list:
        ...
        # We're always inside the Singularity container when called via run-gurobi.bash
        # Use mpirun which will work correctly inside the container with the MPI setup
        # The container's MPI is configured to work with SLURM allocations
    ...
    # Print summary
    print(f"\n{'='*60}")
    print(f"Summary: {len(completed)} completed, {len(failed)} failed, {len(timed_out)} timed out")
    if failed:
        print(f"Failed sizes: {', '.join(failed)}")
    if timed_out:
        print(f"Timed out sizes: {', '.join(timed_out)}")
    print(f"{'='*60}")
