#!/usr/bin/env python3
"""
Medium-scale speed test between standard and 1slack formulations
Tests with 20 items and 1000-2000 agents to see how they scale.
"""
...
def run_medium_scale_test(subproblem_type, num_agents, num_items, num_features, num_simuls, max_iters=50):
    """Run medium-scale speed test for a single configuration."""
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    
    if rank == 0:
        print(f"🚀 MEDIUM-SCALE TEST: {subproblem_type.upper()} - {num_agents} agents, {num_items} items")
        print("=" * 80)
...
def main():
    """Run medium-scale speed tests."""
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    
    if rank == 0:
        print("🚀 MEDIUM-SCALE SPEED COMPARISON: Standard vs 1slack Formulations")
        print("=" * 80)
    
    # Test configurations for medium-scale problems
    test_configs = [
        # (subproblem, agents, items, features, simuls, max_iters)
        ("greedy", 1000, 20, 6, 25, 50),
        ("greedy", 2000, 20, 6, 25, 50),
        ("linear_knapsack", 1000, 20, 6, 25, 50),
        ("linear_knapsack", 2000, 20, 6, 25, 50),
        ("plain_single_item", 1000, 20, 6, 25, 50),
        ("plain_single_item", 2000, 20, 6, 25, 50),
    ]
    
    all_results = []
    
    for config in test_configs:
        result = run_medium_scale_test(*config)
        if result:
