#!/usr/bin/env python3
"""
Simple large-scale speed test focusing only on greedy algorithm
Uses the working configuration from clear_comparison.py
"""
...
def features_oracle(i_id, B_j, data):
    ...
    # For 6 features, we need to create exactly 6 features from the items
    # Take the first 5 items for modular features and use bundle size for the 6th
    modular_subset = modular_agent[:, :5]  # Take first 5 items
    with np.errstate(divide='ignore', invalid='ignore', over='ignore'):
        agent_sum = modular_subset.T @ B_j[:5]  # Use first 5 items
...
def run_speed_test(num_agents, num_items, num_features, num_simuls, max_iters=100):
    ...
    # Generate synthetic data (same as clear_comparison.py)
    np.random.seed(42)
    ...
    # Configuration (same as clear_comparison.py)
    config_gen = {
        "dimensions": {
...
def main():
    ...
    if rank == 0:
        print("🚀 LARGE-SCALE SPEED COMPARISON: Standard vs 1slack (Greedy Only)")
        print("=" * 80)
    
    # Test configurations - start with smaller sizes and work up
    test_configs = [
        # (agents, items, features, simuls, max_iters)
        (100, 10, 6, 25, 50),      # Small baseline
        (500, 10, 6, 25, 50),      # Medium
        (1000, 10, 6, 25, 50),     # Large
        (2000, 10, 6, 25, 50),     # Very large
        (5000, 10, 6, 25, 50),     # Huge
    ]
