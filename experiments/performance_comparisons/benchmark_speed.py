#!/usr/bin/env python3
"""
Speed benchmark comparison between standard and 1slack row generation formulations
Tests different problem sizes and subproblem types to determine which is faster.
"""
...
def run_benchmark(subproblem_type, num_agents, num_items, num_features, num_simuls, max_iters=50):
    """Run speed benchmark for a single configuration."""
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    
    if rank == 0:
        print(f"\n{'='*80}")
        print(f"BENCHMARK: {subproblem_type.upper()} - {num_agents} agents, {num_items} items")
        print(f"{'='*80}")
    ...
    # Configuration for both formulations
    config = {
        "dimensions": {
    ...
    try:
        ...
        standard_iterations = getattr(solver_standard, 'iterations', 'N/A')
    ...
    try:
        ...
        slack_iterations = getattr(solver_1slack, 'iterations', 'N/A')
        
        results['1slack'] = {
            'time': slack_time,
            'objective': slack_obj,
            'constraints': slack_constraints,
            'iterations': slack_iterations,
            'success': True
        }
...
def main():
    ...
    if rank == 0:
        print("🚀 SPEED BENCHMARK: Standard vs 1slack Formulations")
        print("=" * 80)
    ...
    test_configs = [
        ...
        ("greedy", 20, 5, 6, 10, 30),
        ("greedy", 50, 10, 6, 25, 50),
        ("greedy", 100, 15, 6, 50, 100),
        ("linear_knapsack", 20, 5, 6, 10, 30),
        ("linear_knapsack", 50, 10, 6, 25, 50),
        ("linear_knapsack", 100, 15, 6, 50, 100),
        ("plain_single_item", 20, 5, 6, 10, 30),
        ("plain_single_item", 50, 10, 6, 25, 50),
        ("plain_single_item", 100, 15, 6, 50, 100),
    ]
    ...
    if rank == 0 and all_results:
        ...
        print(f"{'Subproblem':<20} {'Size':<15} {'Standard':<10} {'1slack':<10} {'Speedup':<10} {'Winner':<10}")
        print("-" * 80)
        
        for result in all_results:
            size = f"{result['num_agents']}x{result['num_items']}"
            print(f"{result['subproblem']:<20} {size:<15} {result['standard_time']:<10.2f} {result['slack_time']:<10.2f} {result['speedup']:<10.2f} {result['winner']:<10}")
