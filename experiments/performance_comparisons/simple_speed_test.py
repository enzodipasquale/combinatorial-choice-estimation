#!/usr/bin/env python3
"""
Simple speed test between standard and 1slack formulations
"""
...
def run_speed_test(subproblem_type, num_agents=50, num_items=10, num_features=6, num_simuls=25, max_iters=50):
    """Run speed test for a single configuration."""
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    
    if rank == 0:
        print(f"🚀 SPEED TEST: {subproblem_type.upper()} - {num_agents} agents, {num_items} items")
        print("=" * 60)
...
def main():
    """Run speed tests for different subproblems."""
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    
    if rank == 0:
        print("🚀 SPEED COMPARISON: Standard vs 1slack Formulations")
        print("=" * 60)
    
    # Test different subproblems
    test_configs = [
    ...
    if rank == 0 and all_results:
        ...
        print(f"{'Subproblem':<20} {'Standard':<10} {'1slack':<10} {'Speedup':<10} {'Winner':<10}")
        print("-" * 60)
        
        for result in all_results:
            print(f"{result['subproblem']:<20} {result['standard_time']:<10.2f} {result['slack_time']:<10.2f} {result['speedup']:<10.2f} {result['winner']:<10}")
