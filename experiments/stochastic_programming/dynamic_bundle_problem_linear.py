#!/usr/bin/env python3
"""
Dynamic bundle choice problem with additions and removals - LINEAR VERSION (no quadratic terms).

Solves the stochastic programming formulation with explicit scenario tree but WITHOUT
quadratic interaction terms. Pure MILP formulation.
"""
...
class DynamicBundleProblemLinear:
    """
    Solves dynamic bundle choice problem with scenario tree formulation - LINEAR VERSION.
    
    No quadratic terms - pure MILP.
    """
    ...
    def __init__(
        ...
        """
        Initialize dynamic bundle problem (linear version).
        ...
        # Problem data (to be set) - NO S matrix needed
    ...
    def set_data(
        ...
        """
        Set problem data (NO S matrix - no quadratic terms).
    ...
    def solve(self) -> Tuple[Dict[int, np.ndarray], Dict[int, np.ndarray], float]:
        """
        Solve the dynamic bundle problem (linear version).
        ...
        # Create Gurobi model
        model = gp.Model("DynamicBundleLinear")
        ...
        # Parameters optimized for MILP (no NonConvex needed)
        ...
        # Decision variables for each node (binary only, no quadratic terms)
        ...
        # LINEAR VERSION: No quadratic terms
        obj_terms = []
        ...
        for t in range(1, self.num_periods + 1):
            for node in self.nodes_by_time[t]:
                ...
                period_obj = b_linear + d_linear + e_linear + eta_term
...
def generate_test_data_linear(
    num_nodes: int,
    num_actual_items: int,
    seed: int = 42,
) -> Dict[str, np.ndarray]:
    """
    Generate test data for dynamic bundle problem (linear version - no S matrix).
    ...
    Returns:
        Dictionary with keys: r, c, F, eta, b0 (NO S)
    """
    ...
    # NO S matrix - no quadratic interactions
...
def main():
    """Test the dynamic bundle problem solver (linear version)."""
    # Problem parameters
    num_actual_items = 40
    num_periods = 3
    branching_factor = 5
    ...
    print("=" * 80)
    print("Dynamic Bundle Choice Problem (Scenario Tree) - LINEAR VERSION")
    print("=" * 80)
    ...
    print("NOTE: No quadratic terms (pure MILP)")
    print()
    
    # Create problem and build tree
    problem = DynamicBundleProblemLinear(
    ...
    data = generate_test_data_linear(num_nodes, num_actual_items, seed)
    ...
    # Check for trivial/pathological cases
    is_trivial = False
    trivial_reason = []
    
    if all_zero_b:
        is_trivial = True
        trivial_reason.append("all bundles empty")
    if all_one_b:
        is_trivial = True
        trivial_reason.append("all bundles full")
    if all_zero_d and all_zero_e:
        is_trivial = True
        trivial_reason.append("no additions or removals")
    if all_one_d and all_one_e:
        is_trivial = True
        trivial_reason.append("all additions and removals")
    
    # Check if all nodes have same bundle state
    if b_counts:
        all_same_b = len(set(b_counts)) == 1
        if all_same_b:
            is_trivial = True
            trivial_reason.append(f"all nodes have same bundle size ({b_counts[0]})")
    
    # Check if all nodes have same decisions
    if d_counts and e_counts:
        all_same_d = len(set(d_counts)) == 1
        all_same_e = len(set(e_counts)) == 1
        if all_same_d and all_same_e:
            is_trivial = True
            trivial_reason.append("all nodes have identical decisions")
    
    print(f"Solution is {'TRIVIAL' if is_trivial else 'NON-TRIVIAL'}")
    if is_trivial and trivial_reason:
        print(f"  Reason(s): {', '.join(trivial_reason)}")
    print()
