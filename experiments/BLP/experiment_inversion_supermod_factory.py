"""
Row Generation Quadratic Supermodular Experiment with Endogeneity (Factory-based)
...
def run_row_generation_supermod_experiment():
    """Run the row generation quadratic supermodular experiment with endogeneity."""
    ...
    quadratic_mask_threshold = 0.3  # Threshold for masking (for exponential method)
    ...
    for attempt in range(max_retries):
        scenario = (
            ...
            .with_quadratic_method(
                ...
                binary_value=quadratic_binary_value,
                mask_threshold=quadratic_mask_threshold,
            )
            .with_endogeneity(
        ...
        if rank == 0:
            if len(never_chosen) > 0:
                ...
                # Adjust: increase sigma to add more noise/variation
                sigma_adjusted = min(10.0, sigma_adjusted + 1.0)
            if len(always_chosen) > 0:
                ...
                # Adjust: decrease sigma to reduce noise
                sigma_adjusted = max(2.0, sigma_adjusted - 1.0)
            
            if has_issue:
                if attempt < max_retries - 1:
                    print(f"   Item demands: min={item_demands.min()}, max={item_demands.max()}, mean={item_demands.mean():.2f}")
                    print(f"   Adjusting parameters: sigma={sigma_adjusted:.2f}")
                else:
                    print(f"   Item demands: min={item_demands.min()}, max={item_demands.max()}, mean={item_demands.mean():.2f}")
        ...
        sigma_adjusted = comm.bcast(sigma_adjusted, root=0)
    ...
    if rank == 0:
        ...
        # Keep quadratic features and other data
        ...
        prepared.config["row_generation"]["min_iters"] = 1
        parameters_to_log = [i for i in range(num_modular_agent_features)] + [-1]  # Log agent features and quadratic
    ...
    if rank == 0:
        ...
        delta_hat = theta_hat_blp[num_modular_agent_features:-num_quadratic_item_features]  # Fixed effects (exclude agent features and quadratic)
        ...
        true_item_feature_coefficient = theta_0[num_modular_agent_features]  # Should be 2.0
        ...
        # Construct IV version: [agent_features..., IV_item_coefficient, quadratic]
        theta_iv = np.concatenate([
            theta_hat_naive[:num_modular_agent_features],  # All agent features from naive estimate
            [iv_coefficient],      # Item feature from IV regression
            [theta_hat_naive[-1]], # Quadratic from naive estimate
        ])
        ...
        for i in range(num_features):
            ...
            if i < num_modular_agent_features:
                param_name = f"Agent modular {i}"
            elif i < num_modular_agent_features + num_modular_item_features:
                param_name = f"Item modular {i - num_modular_agent_features} (endogenous)"
            else:
                param_name = f"Item quadratic {i - num_modular_agent_features - num_modular_item_features}"
            true_val = theta_0[i]
...
if __name__ == "__main__":
    run_row_generation_supermod_experiment()
