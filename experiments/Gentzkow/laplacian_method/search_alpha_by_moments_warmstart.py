...
def main():
    ...
    # Use different seeds for data generation to test robustness
    data_seed = 100  # Changed from 42
    prepared = scenario.prepare(comm=comm, timeout_seconds=600, seed=data_seed)
    ...
    if rank == 0:
        ...
        # Generate i.i.d. errors ONCE for estimation - these will be transformed for each alpha
        # Use DIFFERENT seed from data generation (200) so estimation errors are independent
        ...
        # Extract Q block for period 0 to prepare for correlation computation
        period = 0
        start_idx = period * num_items_per_period
        end_idx = (period + 1) * num_items_per_period
        period_items = num_items_per_period
        ...
        # Generate base i.i.d. errors ONCE - standard normal (variance 1), will be transformed using Cholesky for each alpha
        # The Cholesky transformation will apply the correct variance (sigma^2) and correlation
        base_iid_time_invariant_errors = generator_estimation.rng.normal(
            0, 1.0, (num_agents, period_items)  # Standard normal, variance = 1
        )
    ...
    for alpha_idx, alpha in enumerate(alpha_values):
        ...
        if rank == 0:
            # Transform base i.i.d. errors using correlation structure based on alpha
            # Compute correlation matrix for this alpha
            ...
            # Transform base i.i.d. errors using Cholesky decomposition
            # Covariance matrix = sigma^2 * corr_matrix
            cov_matrix = sigma_time_invariant**2 * corr_matrix
            try:
                L_chol = np.linalg.cholesky(cov_matrix)
            except np.linalg.LinAlgError:
                # If Cholesky fails, use independent errors
                L_chol = np.eye(period_items) * sigma_time_invariant
            
            # Transform: correlated_errors = L_chol @ base_iid_errors.T
            # base_iid_time_invariant_errors is (num_agents, period_items)
            # We want to transform each agent's errors
            time_invariant_errors_period = (L_chol @ base_iid_time_invariant_errors.T).T
