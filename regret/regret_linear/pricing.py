...
def pricing(modular_characteristics_j_k, quadratic_characteristics_j_j_k, weight_j, capacity):
    ...
    model.setObjective( linear_obj , GRB.MAXIMIZE)
    ...
    characteristics_star = (modular_characteristics_j_k[optimal_bundle]).sum(0)
    toc = time.time()
    # Save results
    return value, characteristics_star, optimal_bundle, toc - tic
