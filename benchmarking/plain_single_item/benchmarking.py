...
def load_and_prepare_data():
    ...
    # File paths
    current_results = Path("benchmarking/plain_single_item/results.csv")
    score_estimator_results = Path("/Users/enzo-macbookpro/MyProjects/score-estimator/plain_single_item/estimation_results.csv")
...
def boxplot_normalized(norm_betas_df, score_betas, results_folder):
    ...
    legend_elements = [Patch(facecolor='lightblue', alpha=0.7, label='Current (normalized)'),
                      Patch(facecolor='lightcoral', alpha=0.7, label='Score estimator')]
    ax.legend(handles=legend_elements)
    
    ax.set_ylabel('Normalized Beta Estimate')
    ax.set_title('Boxplot of Normalized Beta Estimates (k-1)')
...
def create_results_folder():
    ...
    results_folder = Path("benchmarking/plain_single_item/comparison_results")
