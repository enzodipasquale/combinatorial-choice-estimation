...
def initialize_pb(modular_characteristics_i_j_k, quadratic_characteristic_j_j_k, matching_i_j):
    ...
    model = gp.Model(f"GMM_pb_{num_simulations}")
    ...
    # add non negativity constraint lambda_k[2]>=0
    model.addConstr(lambda_k[num_characteristics -1] >= 0, name='non_negativity_0')
    model.addConstr(lambda_k[num_characteristics -2] >= 0, name='non_negativity_1')
    

    # Constraints
    model.addConstrs((
    u_si[si] + gp.quicksum(matching_i_j[si % num_agents, j] * p_j[j] for j in range(num_objects)) 
    >= epsilon_si_j[si, matching_i_j[si % num_agents]].sum()  # Ensure this slice/indexing is correct
    + gp.quicksum(phi_hat_i_k[si % num_agents, k] * (1 + 5 * 1e-7) * lambda_k[k] for k in range(num_characteristics))
